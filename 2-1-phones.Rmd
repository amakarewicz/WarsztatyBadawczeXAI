...
Given the type of the model (blackbox i.e. non-interpretable), we use Explainable Artificial Intelligence (XAI) methods, for both local and global explanations, to interpret its predictions and get to know which features influence them the most, and raise (or lower) the price of the phone.
...
The next step after EDA was creating prediction models. To compare results and find the best model for mentioned data, there were created 3 models: ranger, XGBoost and SVM. Every model used cross-validation during training. SVM and XGBoost models cannot be trained using categorical variables so variable `brand` needed to be target encoded in these cases. There were used two measures to compare models results: root mean square error (RMSE) and mean absolute error (MAE). The results are presented in the table below.
